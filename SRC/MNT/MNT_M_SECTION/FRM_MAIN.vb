...
#Region "初期化・終了処理"
    ...
    Private Sub SUB_CTRL_VIEW_INIT()
        Call glbSubMakeDataTable(TBL_GRID_DATA_MAIN, "部署コード,部署名称,削除", "SSS")
        DGV_VIEW_DATA.DataSource = TBL_GRID_DATA_MAIN
        Call SUB_DGV_COLUMN_WIDTH_INIT_COUNT_FONT(DGV_VIEW_DATA, "4,9,2", "RLC")
    End Sub
...
#Region "実行処理群"
    ...
    Private Sub SUB_SELECT_SEQ()
        ...
        INT_CODE_SECTION = FUNC_GET_SELECT_RET_CODE()
        If INT_CODE_SECTION <= 0 Then
            Exit Sub
        End If

        Call SUB_CLEAR()

        TXT_CODE_SECTION.Text = INT_CODE_SECTION
        Call TXT_CODE_SECTION.Focus()
        Call Application.DoEvents()
    ...
    Private Sub SUB_DATA_EDIT()
        ...
        Dim SRT_KEY As SRT_TABLE_MNT_M_SECTION_KEY
        SRT_KEY = FUNC_GET_INPUT_KEY()

        Dim BLN_CHECK As Boolean
        BLN_CHECK = FUNC_CHECK_TABLE_MNT_M_SECTION(SRT_KEY)
    ...
    Private Sub SUB_ENTER()
        ...
        Dim SRT_RECORD As SRT_TABLE_MNT_M_SECTION
...
#Region "グリッド関連"
    ...
    Private Sub SUB_MAKE_GRID_DATA(ByVal SRT_CONDITIONS As SRT_SEARCH_CONDITIONS)
        ...
        With STR_SQL
            ...
            Call .Append("MNT_M_SECTION WITH(NOLOCK)" & Environment.NewLine)
        ...
        For i = 1 To (SRT_GRID_DATA_MAIN.Length - 1) '補助情報取得
            With SRT_GRID_DATA_MAIN(i)
                .FLAG_INVALID_NAME = FUNC_GET_MNT_M_KIND_NAME_KIND(ENM_MNT_M_KIND_CODE_FLAG.FLAG_INVALID_SHORT, .FLAG_INVALID)
            End With
        Next
    ...
    Private Sub SUB_REFRESH_GRID()
        ...
        For i = 1 To INT_MAX_INDEX
            With SRT_GRID_DATA_MAIN(i)
                OBJ_TEMP(ENM_MY_GRID_MAIN.CODE_SECTION) = Format(.CODE_SECTION, New String("0", INT_SYSTEM_CODE_SECTION_MAX_LENGTH))
                OBJ_TEMP(ENM_MY_GRID_MAIN.NAME_SECTION) = .NAME_SECTION
    ...
    Private Function FUNC_GET_SELECT_RET_CODE() As Integer
        ...
        With SRT_GRID_DATA_MAIN(INT_SRT_INDEX)
            INT_RET = FUNC_VALUE_CONVERT_NUMERIC_INT(.CODE_SECTION)
        End With
        Return INT_RET
    End Function
#End Region
...
#Region "画面コントロール←→構造体"
    Private Function FUNC_GET_INPUT_KEY() As SRT_TABLE_MNT_M_SECTION_KEY
        Dim SRT_RET As SRT_TABLE_MNT_M_SECTION_KEY

        With SRT_RET
            .CODE_SECTION = CInt(TXT_CODE_SECTION.Text)
        End With

        Return SRT_RET
    End Function

    Private Function FUNC_GET_INPUT_DATA() As SRT_TABLE_MNT_M_SECTION_DATA
        Dim SRT_RET As SRT_TABLE_MNT_M_SECTION_DATA
    ...
    Private Sub SUB_SET_INPUT_DATA(ByRef SRT_DATA As SRT_TABLE_MNT_M_SECTION_DATA)
        With SRT_DATA
            TXT_NAME_SECTION.Text = .NAME_SECTION
...
#Region "キー制御処理"
    ...
    Private Function FUNC_RETURN_KEYDOWN() As Boolean
        ...
        Select Case True
            Case (CTL_ACTIVE Is TXT_CODE_SECTION)
