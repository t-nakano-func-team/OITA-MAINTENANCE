...
#Region "初期化・終了処理"
    ...
    Private Sub SUB_CTRL_VIEW_INIT()
        Call glbSubMakeDataTable(TBL_GRID_DATA_MAIN, "作業担当コード,作業担当名称,作業区分,部署", "SSSS")
        DGV_VIEW_DATA.DataSource = TBL_GRID_DATA_MAIN
        Call SUB_DGV_COLUMN_WIDTH_INIT_COUNT_FONT(DGV_VIEW_DATA, "5,10,5,5", "RLLL")
...
#Region "実行処理群"
    ...
    Private Sub SUB_SELECT_SEQ()
        ...
        Dim INT_CODE_HANDLE As Integer
        INT_CODE_HANDLE = FUNC_GET_SELECT_RET_CODE()
        If INT_CODE_HANDLE <= 0 Then
            Exit Sub
        End If

        Call SUB_CLEAR()

        TXT_CODE_HANDLE.Text = INT_CODE_HANDLE
        Call TXT_CODE_HANDLE.Focus()
        Call Application.DoEvents()
    ...
    Private Sub SUB_DATA_EDIT()
        ...
        Dim SRT_KEY As SRT_TABLE_MNT_M_HANDLE_WORK_KEY
        SRT_KEY = FUNC_GET_INPUT_KEY()

        Dim BLN_CHECK As Boolean
        BLN_CHECK = FUNC_CHECK_TABLE_MNT_M_HANDLE_WORK(SRT_KEY)

        Dim ENM_CHANGE_MODE As ENM_MY_WINDOW_MODE
        If BLN_CHECK Then
            Dim SRT_DATA As SRT_TABLE_MNT_M_HANDLE_WORK_DATA
            SRT_DATA = Nothing
            Call FUNC_SELECT_TABLE_MNT_M_HANDLE_WORK(SRT_KEY, SRT_DATA)
    ...
    Private Sub SUB_ENTER()
        ...
        Dim SRT_RECORD As SRT_TABLE_MNT_M_HANDLE_WORK
...
#Region "登録内部処理"
    ...
    Private Function FUNC_EDIT_RECORD(ByRef SRT_RECORD As SRT_TABLE_MNT_M_HANDLE_WORK) As Boolean

        If Not FUNC_DELETE_TABLE_MNT_M_HANDLE_WORK(SRT_RECORD.KEY) Then
            STR_FUNC_EDIT_RECORD_LAST_ERROR = FUNC_SYSTEM_SQLGET_ERR_MESSAGE()
            Return False
        End If

        If Not FUNC_INSERT_TABLE_MNT_M_HANDLE_WORK(SRT_RECORD) Then
            STR_FUNC_EDIT_RECORD_LAST_ERROR = FUNC_SYSTEM_SQLGET_ERR_MESSAGE()
            Return False
        End If
...
#End Region
    ...
    Private Sub SUB_DELETE()
        ...
        '物理削除
        If Not FUNC_DELETE_TABLE_MNT_M_HANDLE_WORK(SRT_KEY) Then
            Call MessageBox.Show(FUNC_SYSTEM_SQLGET_ERR_MESSAGE(), Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
            Call FUNC_SYSTEM_ROLLBACK_TRANSACTION()
            Exit Sub
        End If
...
#Region "グリッド関連"
    ...
    Private Sub SUB_MAKE_GRID_DATA(ByVal SRT_CONDITIONS As SRT_SEARCH_CONDITIONS)
        ...
        With STR_SQL
            ...
            Call .Append("MNT_M_HANDLE_WORK WITH(NOLOCK)" & Environment.NewLine)
    ...
    Private Sub SUB_REFRESH_GRID()
        ...
        For i = 1 To INT_MAX_INDEX
            With SRT_GRID_DATA_MAIN(i)
                OBJ_TEMP(ENM_MY_GRID_MAIN.CODE_HANDLE) = Format(.CODE_HANDLE, New String("0", INT_SYSTEM_CODE_HANDLE_MAX_LENGTH))
                OBJ_TEMP(ENM_MY_GRID_MAIN.NAME_HANDLE) = .NAME_HANDLE
    ...
    Private Function FUNC_GET_SELECT_RET_CODE() As Integer
        ...
        With SRT_GRID_DATA_MAIN(INT_SRT_INDEX)
            INT_RET = FUNC_VALUE_CONVERT_NUMERIC_INT(.CODE_HANDLE)
        End With
        Return INT_RET
    End Function

    Private Sub SUB_SET_INDEX_GRID_FROM_KEY(ByVal SRT_KEY As SRT_TABLE_MNT_M_HANDLE_WORK_KEY)
        Dim INT_INDEX As Integer
    ...
    Private Function FUNC_GET_INDEX_FROM_GRID(ByVal SRT_KEY As SRT_TABLE_MNT_M_HANDLE_WORK_KEY)
        ...
        For i = 1 To (SRT_GRID_DATA_MAIN.Length - 1)
            With SRT_GRID_DATA_MAIN(i)
                If .CODE_HANDLE = SRT_KEY.CODE_HANDLE Then
                    Return i
                End If
            End With
        Next
...
#Region "画面コントロール←→構造体"
    Private Function FUNC_GET_INPUT_KEY() As SRT_TABLE_MNT_M_HANDLE_WORK_KEY
        Dim SRT_RET As SRT_TABLE_MNT_M_HANDLE_WORK_KEY

        With SRT_RET
            .CODE_HANDLE = CInt(TXT_CODE_HANDLE.Text)
        End With

        Return SRT_RET
    End Function

    Private Function FUNC_GET_INPUT_DATA() As SRT_TABLE_MNT_M_HANDLE_WORK_DATA
        Dim SRT_RET As SRT_TABLE_MNT_M_HANDLE_WORK_DATA

        With SRT_RET
            .NAME_HANDLE = CStr(TXT_NAME_HANDLE.Text)
    ...
    Private Sub SUB_SET_INPUT_DATA(ByRef SRT_DATA As SRT_TABLE_MNT_M_HANDLE_WORK_DATA)
        With SRT_DATA
            TXT_NAME_HANDLE.Text = .NAME_HANDLE
...
#Region "キー制御処理"
    ...
    Private Function FUNC_RETURN_KEYDOWN() As Boolean
        ...
        Select Case True
            Case (CTL_ACTIVE Is TXT_CODE_HANDLE)
