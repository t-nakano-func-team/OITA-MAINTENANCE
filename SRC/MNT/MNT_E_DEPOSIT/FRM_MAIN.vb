...
#Region "画面用・変数"
    ...
    Private SRT_CONDITIONS_BEFORE As SRT_SEARCH_CONDITIONS
...
#Region "初期化・終了処理"
    ...
    Private Sub SUB_CTRL_VIEW_INIT()
        ...
        '初期日付をセット
        With SRT_CONDITIONS_BEFORE
            .DATE_INVOICE_FROM = FUNC_GET_DATE_FIRSMONTH(datSYSTEM_TOTAL_DATE_ACTIVE.AddMonths(-2))
            .DATE_INVOICE_TO = FUNC_GET_DATE_LASTMONTH(datSYSTEM_TOTAL_DATE_ACTIVE)
            .FLAG_CONTRACT = -1
            .FLAG_DEPOSIT_DONE = ENM_SYSTEM_INDIVIDUAL_FLAG_DEPOSIT_DONE.NOT_DONE
        End With

        '初期入力値をセット
        DAT_DATE_DEPOSIT_INIT_VALUE = datSYSTEM_TOTAL_DATE_ACTIVE
        INT_FLAG_SALE_INIT_VALUE = -1
        INT_FLAG_DEPOSIT_INIT_VALUE = -1
        INT_FLAG_DEPOSIT_SUB_INIT_VALUE = -1
    End Sub
    ...
    Private Sub SUB_CTRL_VALUE_INIT()
        ...
        Call SUB_CONTROL_SET_VALUE_DateTimePicker(DTP_DATE_INVOICE_FROM, SRT_CONDITIONS_BEFORE.DATE_INVOICE_FROM)
        Call SUB_CONTROL_SET_VALUE_DateTimePicker(DTP_DATE_INVOICE_TO, SRT_CONDITIONS_BEFORE.DATE_INVOICE_TO)

        Call SUB_SET_COMBO_KIND_CODE(CMB_FLAG_CONTRACT, SRT_CONDITIONS_BEFORE.FLAG_CONTRACT)
        Call SUB_REFRESH_ENABLED_NAME_OWNER()
        Call SUB_SET_COMBO_KIND_CODE(CMB_FLAG_DEPOSIT_DONE, SRT_CONDITIONS_BEFORE.FLAG_DEPOSIT_DONE)
...
#Region "実行処理群"
    ...
    Private Sub SUB_SEARCH()
        ...
        SRT_CONDITIONS_BEFORE = SRT_CONDITIONS
...
#Region "グリッド関連"
    Private Sub SUB_MAKE_GRID_DATA(ByVal SRT_CONDITIONS As SRT_SEARCH_CONDITIONS)
        ...
        With STR_SQL
            ...
            Call .Append("SELECT" & Environment.NewLine)
            Call .Append("MAIN.*" & "," & Environment.NewLine)
            Call .Append("SUB_01.FLAG_CONTRACT" & "," & Environment.NewLine)
...
#Region "内部処理"
    ...
    Private Sub SUB_REFRESH_GRID_ROW(ByVal INT_INDEX_SRT As Integer)
        ...
        With SRT_GRID_DATA_MAIN(INT_INDEX_SRT)
            ...
            TBL_GRID_DATA_MAIN.Rows(INT_INDEX_GRID).Item(ENM_MY_GRID_MAIN.FLAG_CONTRACT_NAME) = .FLAG_CONTRACT_NAME
...
#Region "その他処理"
    Private Sub SUB_REFRESH_ENABLED_NAME_OWNER()
        Dim ENM_FLAG_CONTRACT As ENM_SYSTEM_INDIVIDUAL_FLAG_CONTRACT
        ENM_FLAG_CONTRACT = FUNC_GET_COMBO_KIND_CODE(CMB_FLAG_CONTRACT)
        Dim BLN_ENABLED As Boolean
        If ENM_FLAG_CONTRACT = ENM_SYSTEM_INDIVIDUAL_FLAG_CONTRACT.SPOT Then
            BLN_ENABLED = True
        Else
            BLN_ENABLED = False
        End If
...
#Region "イベント-セレクトインデックスチェンジ"
    Private Sub CMB_FLAG_CONTRACT_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CMB_FLAG_CONTRACT.SelectedIndexChanged
        Call SUB_REFRESH_ENABLED_NAME_OWNER()
    End Sub
#End Region
