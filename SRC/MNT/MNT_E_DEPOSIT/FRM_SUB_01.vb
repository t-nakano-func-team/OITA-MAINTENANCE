...
#Region "画面用・変数"
    ...
    Private LNG_KINGAKU_INVOICE_DETAIL_CALC_VAT As Long
#End Region
...
#Region "初期化・終了処理"
    ...
    Private Sub SUB_CTRL_VIEW_INIT()
        ...
        Call SUB_SYSTEM_COMMBO_MNT_M_ACCOUNT(CMB_FLAG_SALE, ENM_SYSTEM_INDIVIDUAL_FLAG_ACCOUNT.FLAG_SALE)
        Call SUB_SYSTEM_COMMBO_MNT_M_ACCOUNT(CMB_FLAG_DEPOSIT, ENM_SYSTEM_INDIVIDUAL_FLAG_ACCOUNT.FLAG_DEPOSIT)
        Call SUB_SYSTEM_COMMBO_MNT_M_ACCOUNT(CMB_FLAG_DEPOSIT_SUB, ENM_SYSTEM_INDIVIDUAL_FLAG_ACCOUNT.KIND_PAYEE)
        Call SUB_SYSTEM_COMMBO_MNT_M_ACCOUNT(CMB_FLAG_COST, ENM_SYSTEM_INDIVIDUAL_FLAG_ACCOUNT.FLAG_COST, True, "なし")
    End Sub
    ...
    Private Sub SUB_CTRL_VALUE_INIT()
        ...
        LNG_KINGAKU_INVOICE_DETAIL_CALC_VAT = -1
...
#Region "画面コントロール←→構造体"
    ...
    Private Sub SUB_SET_INPUT_KEY(ByRef SRT_DATA As SRT_TABLE_MNT_T_INVOICE_KEY)
        ...
        With SRT_DATA
            ...
            Select Case SRT_CONTRACT.DATA.FLAG_CONTRACT
    ...
    Private Sub SUB_SET_INPUT_DATA_DEPOSIT(ByRef SRT_DATA As SRT_TABLE_MNT_T_DEPOSIT_DATA)
        With SRT_DATA
            If CHK_FLAG_DEPOSIT_DONE.Checked Then
                ...
                Call SUB_SET_COMBO_KIND_CODE(CMB_FLAG_SALE, .FLAG_SALE)
                Call SUB_SET_COMBO_KIND_CODE(CMB_FLAG_DEPOSIT, .FLAG_DEPOSIT)
                Call SUB_REFRSH_ENABLED_FLAG_DEPOSIT_SUB()
                Call SUB_SET_COMBO_KIND_CODE(CMB_FLAG_DEPOSIT_SUB, .FLAG_DEPOSIT_SUB)
                ...
                Call SUB_SET_COMBO_KIND_CODE(CMB_FLAG_COST, .FLAG_COST)
            ...
            Else
                ...
                Call SUB_REFRESH_KINGAKU_FEE_TOTAL()
                Call SUB_SET_COMBO_KIND_CODE_FIRST(CMB_FLAG_COST)
                Call SUB_REFRSH_ENABLED_KINGAKU_COST()
    ...
    Private Function FUNC_GET_INPUT_DATA_DEPOSIT() As SRT_TABLE_MNT_T_DEPOSIT_DATA
        ...
        With SRT_RET
            ...
            .FLAG_SALE = FUNC_GET_COMBO_KIND_CODE(CMB_FLAG_SALE)
            .FLAG_DEPOSIT = FUNC_GET_COMBO_KIND_CODE(CMB_FLAG_DEPOSIT)
            .FLAG_DEPOSIT_SUB = FUNC_GET_COMBO_KIND_CODE(CMB_FLAG_DEPOSIT_SUB)
            .KINGAKU_FEE_DETAIL = CLng(CMB_KINGAKU_FEE_DETAIL.Text)
            .KINGAKU_FEE_VAT = CLng(TXT_KINGAKU_FEE_VAT.Text)
            .FLAG_COST = FUNC_GET_COMBO_KIND_CODE(CMB_FLAG_COST)
            .KINGAKU_COST_DETAIL = CLng(TXT_KINGAKU_COST_DETAIL.Text)
            .KINGAKU_COST_VAT = CLng(TXT_KINGAKU_COST_VAT.Text)
            .FLAG_OUTPUT = ENM_SYSTEM_INDIVIDUAL_FLAG_DEPOSIT_DONE.NOT_DONE
...
#Region "内部処理"
    Private Sub SUB_REFRESH_KINGAKU_INVOICE_VAT()
        ...
        If LNG_KINGAKU_INVOICE_DETAIL_CALC_VAT = LNG_KINGAKU_INVOICE_DETAIL Then
            Exit Sub
        End If
        ...
        LNG_KINGAKU_INVOICE_DETAIL_CALC_VAT = LNG_KINGAKU_INVOICE_DETAIL
    End Sub
    ...
    Private Sub SUB_REFRSH_ENABLED_KINGAKU_COST()
        Dim INT_FLAG_COST As Integer
        INT_FLAG_COST = FUNC_GET_COMBO_KIND_CODE(CMB_FLAG_COST)

        Dim BLN_ENABLED As Boolean
        If INT_FLAG_COST <= 0 Then
            BLN_ENABLED = False
        Else
            BLN_ENABLED = True
        End If
    ...
    Private Sub SUB_REFRSH_ENABLED_FLAG_DEPOSIT_SUB()
        Dim INT_FLAG_DEPOSIT As Integer
        INT_FLAG_DEPOSIT = FUNC_GET_COMBO_KIND_CODE(CMB_FLAG_DEPOSIT)

        Dim BLN_ENABLED As Boolean
        If INT_FLAG_DEPOSIT = CST_SYSTEM_ACCOUNT_CODE_KIND_ORDINARY_DEPOSIT Then
            BLN_ENABLED = True
        Else
            BLN_ENABLED = False
        End If
        ...
        If BLN_ENABLED Then
            Call SUB_SET_COMBO_KIND_CODE_FIRST(CMB_FLAG_DEPOSIT_SUB)
        Else
            Call SUB_SET_COMBO_KIND_CODE(CMB_FLAG_DEPOSIT_SUB, -1)
        End If
        PNL_FLAG_DEPOSIT_SUB.Enabled = BLN_ENABLED
...
#Region "イベント-セレクトインデックスチェンジ"
    ...
    Private Sub CMB_FLAG_DEPOSIT_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CMB_FLAG_DEPOSIT.SelectedIndexChanged
        Call SUB_REFRSH_ENABLED_FLAG_DEPOSIT_SUB()
    End Sub

    Private Sub CMB_FLAG_COST_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CMB_FLAG_COST.SelectedIndexChanged
        Call SUB_REFRSH_ENABLED_KINGAKU_COST()
    End Sub
#End Region
