...
#Region "帳票用・構造体"
    ...
    Public Structure SRT_PRINT_DATA '印刷データ
        ...
        Public Function SORT_INDEX() As Decimal
            ...
            Const CST_FLAG_CONTRACT_MAX_LENGTH As Integer = 2
            Const CST_DATE_MAX_LENGTH As Integer = 8
            DEC_RET += CDec(Me.CODE_SECTION * (10 ^ CST_FLAG_CONTRACT_MAX_LENGTH) * (10 ^ CST_DATE_MAX_LENGTH) * (10 ^ INT_SYSTEM_CODE_OWNER_MAX_LENGTH) * (10 ^ INT_SYSTEM_NUMBER_CONTRACT_MAX_LENGTH) * (10 ^ INT_SYSTEM_SERIAL_CONTRACT_MAX_LENGTH))
            DEC_RET += CDec(Me.FLAG_CONTRACT * (10 ^ CST_DATE_MAX_LENGTH) * (10 ^ INT_SYSTEM_CODE_OWNER_MAX_LENGTH) * (10 ^ INT_SYSTEM_NUMBER_CONTRACT_MAX_LENGTH) * (10 ^ INT_SYSTEM_SERIAL_CONTRACT_MAX_LENGTH))

            Dim INT_DATE_INVOICE_PLAN As Integer
            INT_DATE_INVOICE_PLAN = FUNC_CONVERT_DATETIME_TO_NUMERIC_DATE(Me.DATE_INVOICE_PLAN)
            DEC_RET += CDec(INT_DATE_INVOICE_PLAN * (10 ^ INT_SYSTEM_CODE_OWNER_MAX_LENGTH) * (10 ^ INT_SYSTEM_NUMBER_CONTRACT_MAX_LENGTH) * (10 ^ INT_SYSTEM_SERIAL_CONTRACT_MAX_LENGTH))

            Dim INT_CODE_OWNER_SORT As Integer
            Select Case Me.FLAG_CONTRACT
                Case ENM_SYSTEM_INDIVIDUAL_FLAG_CONTRACT.REGULAR
                    INT_CODE_OWNER_SORT = Me.CODE_OWNER
                Case ENM_SYSTEM_INDIVIDUAL_FLAG_CONTRACT.SPOT
                    INT_CODE_OWNER_SORT = 0
                Case Else
                    INT_CODE_OWNER_SORT = 0
            End Select
            DEC_RET += CDec(INT_CODE_OWNER_SORT * (10 ^ INT_SYSTEM_NUMBER_CONTRACT_MAX_LENGTH) * (10 ^ INT_SYSTEM_SERIAL_CONTRACT_MAX_LENGTH))

            DEC_RET += CDec(Me.NUMBER_CONTRACT * (10 ^ INT_SYSTEM_SERIAL_CONTRACT_MAX_LENGTH))
            DEC_RET += CDec(Me.SERIAL_CONTRACT)
...
#End Region
    ...
    ByVal BLN_PUT_FILE As Boolean
        ...
        For i = 1 To (SRT_DATA.Length - 1)
            If (INT_CODE_SECTION_BEFORE <> SRT_DATA(i).CODE_SECTION) _
                Or (INT_FLAG_CONTRACT_BEFORE <> SRT_DATA(i).FLAG_CONTRACT) _
    ...
    Private Function FUNC_GET_SQL(ByRef SRT_CONDITIONS As SRT_PRINT_CONDITIONS) As String
        ...
        With STR_SQL
            ...
            Call .Append("MAIN.CODE_SECTION,MAIN.FLAG_CONTRACT,MAIN.NUMBER_CONTRACT,MAIN.SERIAL_CONTRACT" & System.Environment.NewLine)
        End With
